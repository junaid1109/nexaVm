<template>
    <component :is="as" v-if="!to" :type="type" :disabled="disabled" :class="active ? activeClass : undefined">
        <slot v-bind="{ isActive: active }" />
    </component>
    <NuxtLink v-else v-slot="{ route, href, target, rel, navigate, isActive, isExactActive, isExternal }"
        v-bind="$props" custom>
        <a v-bind="$attrs" :href="!disabled ? href : undefined" :aria-disabled="disabled ? 'true' : undefined"
            :role="disabled ? 'link' : undefined" :rel="rel" :target="isExternal ? '_blank' : target || undefined"
